...
dm.Start("127.0.0.1", 9001, function() {console.log("Remote conectado a server")})
...
sub.on('message', function (topic, data) {
	console.log ('(SUB) data comes in: ' + data);
	var reply = JSON.parse(data.toString());
	switch (reply.what) {
		// TODO complete list of commands
		case 'add user':
			console.log ("We received a broadcast message for ADD USER: " + data);
			if (!reply.exists) {
    			io.emit ('new user', 'add', reply.usr);      
    		}
    		break;
		case 'add subject':
			console.log ("We received a broadcast message for ADD SUBJECT: " + data);
			if (reply.id != -1) {
				io.emit ('new subject', 'add', reply.id, reply.sbj);
    		}
			break;
		
		case 'add private message':
		case 'add public message':
			console.log ('We received a broadcast message for ADD MESSAGE: ' + data);
			io.emit('message', JSON.stringify(reply.msg));
			break;
